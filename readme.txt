Installation
------------
1. Download Node.js <https://nodejs.org/en/download/> (Application was tested with version 16.14.0)
2. Navigate to the application folder 
3. Install dependancies:   'npm install'
4. Run the app:            'npm start'



---------- Οδηγίες για εισοδο (Login) με ήδη υπάρχων account ηλεκτρονικού ταχυδρομείου ----------
--------------------------------  (συνήθεις παροχοι)  -------------------------------------------

** Outlook / Microsoft/ Hotmail / Msn ** : Χρήση απλώς της email διεύθυνσης και Password του ήδη υπάρχοντος
                                           account του χρήστη στην υπηρεσία

** Yahoo! ** : Χρήση της ήδη υπάρχουσας email διεύθυνσης του χρήστη στην υπηρεσία. 
               Σχετικά με το password πρέπει να δημιουργηθεί διαφορετικός μοναδικός κωδικός 
               για χρήση αποκλειστικά και μόνο στον παρών client. 
               Βήματα: 1. Στον Yahoo! λογαριασμό σας επιλέξτε 'Add or Manage Accounts' 
                       2. Επιλέξτε 'Account Info'
                       3. Επιλέξτε την καρτέλα 'Account Security'
                       4. Από τις διαθέσιμες επιλογές σύνδεσης μεταβείτε στην 'Other ways to sign in'
                       5. Επιλέξτε 'Generate App Password'
                       6. Όταν σας ζητηθεί το όνομα της εφαρμογής (App Name) χρησιμοποιήστε 
                          το όνομα 'email-client' (χωρίς τα ' ') και επιλέξτε Generate App Password.
                       7. Χρησιμοποιήστε αυτό το νέο Password μαζί με την διεύθυνση email σας για είσοδο
                          (μόνο) στη συγκεκριμένη εφαρμογή (παρών email client).
                          

** Gmail ** :

** Upnet / Upatras ** : 


Σημειωση: Ο παρών client δεν διαθέτει δυνατότητα δημιουργίας λογαρισμού (account) σε οποιαδοίποτε 
          από τις παρακάτω αναφερόμενες υπηρεσίες/ παρόχους.